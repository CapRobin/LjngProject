 ...
 */
package com.zfg.org.myexample.achartengine.model;



import com.zfg.org.myexample.achartengine.util.IndexXYMap;
import com.zfg.org.myexample.achartengine.util.MathHelper;
import com.zfg.org.myexample.achartengine.util.XYEntry;
...
public class XYSeries implements Serializable {
	...
	private static final double PADDING = 0.0000000001;
	...
	public synchronized void add(double x, double y) {
		while (mXY.get(x) != null) {
			// add a very small value to x such as data points sharing the same
			// x will
			// still be added
			if (x == 0) {
				x += getPadding();
	 ...
	 * @param stop
	 *            stop x value
	 * @param beforeAfterPoints
	 *            if the points before and after the first and last visible ones
	 *            must be displayed
	 * @return a submap of x and y values
	 */
	public synchronized SortedMap<Double, Double> getRange(double start,
			double stop, boolean beforeAfterPoints) {
		if (beforeAfterPoints) {
			// we need to add one point before the start and one point after the
			// end
			// (if
			// there are any)
			// to ensure that line doesn't end before the end of the screen

			// this would be simply: start = mXY.lowerKey(start) but
			// NavigableMap is
			...
			// this would be simply: end = mXY.higherKey(end) but NavigableMap
			// is
			// available since API 9
