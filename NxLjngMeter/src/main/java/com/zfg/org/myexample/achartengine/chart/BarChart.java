...
public class BarChart extends XYChart {
  ...
  public void drawSeries(Canvas canvas, Paint paint, List<Float> points,
    ...
    int[] colors = getRenderer().getLineColor();
    float halfDiffX = getHalfDiffX(points, length, seriesNr);
    for (int i = 0; i < length; i += 2) {
      float x = points.get(i);
      float y = points.get(i + 1);
      if(colors != null){
    	  paint.setColor(colors[(i/2) % colors.length]);
      }
  ...
  private void drawBar(Canvas canvas, float xMin, float yMin, float xMax, float yMax, int scale,
    ...
    SimpleSeriesRenderer renderer = mRenderer.get().getSeriesRendererAt(seriesIndex);
  ...
  protected float getHalfDiffX(List<Float> points, int length, int seriesNr) {
    float barWidth = mRenderer.get().getBarWidth();
    if (barWidth > 0) {
      return barWidth / 2;
    }
    int div = length;
    ...
    return (float) (halfDiffX / (getCoeficient() * (1 + mRenderer.get().getBarSpacing())));
  }
