...
public class TimeChart extends LineChart {
    ...
    if (length > 0) {
      ...
      for (int i = 0; i < length; i++) {
        ...
        if (showLabels) {
          ...
          canvas
              .drawLine(xLabel, bottom, xLabel, bottom + mRenderer.get().getLabelsTextSize() / 3, paint);
          drawText(canvas, format.format(new Date(label)), xLabel,
              bottom + mRenderer.get().getLabelsTextSize() * 4 / 3 + mRenderer.get().getXLabelsPadding(), paint, mRenderer.get().getXLabelsAngle());
        }
        if (showGridY) {
  ...
  protected List<Double> getXLabels(double min, double max, int count) {
    ...
    if (!mRenderer.get().isXRoundedLabels()) {
