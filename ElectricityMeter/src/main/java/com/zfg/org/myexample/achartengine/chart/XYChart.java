 ...
 */
package com.zfg.org.myexample.achartengine.chart;
...
import com.zfg.org.myexample.achartengine.model.Point;
import com.zfg.org.myexample.achartengine.model.SeriesSelection;
import com.zfg.org.myexample.achartengine.model.XYMultipleSeriesDataset;
import com.zfg.org.myexample.achartengine.model.XYSeries;
import com.zfg.org.myexample.achartengine.renderer.BasicStroke;
import com.zfg.org.myexample.achartengine.renderer.DefaultRenderer;
import com.zfg.org.myexample.achartengine.renderer.SimpleSeriesRenderer;
import com.zfg.org.myexample.achartengine.renderer.XYMultipleSeriesRenderer;
import com.zfg.org.myexample.achartengine.renderer.XYMultipleSeriesRenderer.Orientation;
import com.zfg.org.myexample.achartengine.renderer.XYSeriesRenderer;
import com.zfg.org.myexample.achartengine.util.MathHelper;
...
public abstract class XYChart extends AbstractChart {
	...
	protected final ThreadLocal<XYMultipleSeriesRenderer> mRenderer = new ThreadLocal<>();
	...
	public void draw(Canvas canvas, int x, int y, int width, int height,
			Paint paint) {
		paint.setAntiAlias(mRenderer.get().isAntialiasing());
		int legendSize = getLegendSize(mRenderer.get(), height / 5,
				mRenderer.get().getAxisTitleTextSize());
		int[] margins = mRenderer.get().getMargins();
		...
		if (mRenderer.get().isFitLegend() && mRenderer.get().isShowLegend()) {
			legendSize = drawLegend(canvas, mRenderer.get(), titles, left, right, y,
		...
		drawBackground(mRenderer.get(), canvas, x, y, width, height, paint, false,
				DefaultRenderer.NO_COLOR);

		if (paint.getTypeface() == null
				|| (mRenderer.get().getTextTypeface() != null && paint.getTypeface()
						.equals(mRenderer.get().getTextTypeface()))
				|| !paint.getTypeface().toString()
						.equals(mRenderer.get().getTextTypefaceName())
				|| paint.getTypeface().getStyle() != mRenderer.get()
						.getTextTypefaceStyle()) {
			if (mRenderer.get().getTextTypeface() != null) {
				paint.setTypeface(mRenderer.get().getTextTypeface());
			} else {
				paint.setTypeface(Typeface.create(
						mRenderer.get().getTextTypefaceName(),
						mRenderer.get().getTextTypefaceStyle()));
			}
		}
		Orientation or = mRenderer.get().getOrientation();
		...
		for (int i = 0; i < maxScaleNumber; i++) {
			minX[i] = mRenderer.get().getXAxisMin(i);
			maxX[i] = mRenderer.get().getXAxisMax(i);
			minY[i] = mRenderer.get().getYAxisMin(i);
			maxY[i] = mRenderer.get().getYAxisMax(i);
			isMinXSet[i] = mRenderer.get().isMinXSet(i);
			isMaxXSet[i] = mRenderer.get().isMaxXSet(i);
			isMinYSet[i] = mRenderer.get().isMinYSet(i);
			isMaxYSet[i] = mRenderer.get().isMaxYSet(i);
		...
		// areas
		drawBackground(mRenderer.get(), canvas, x, bottom, width, height - bottom,
				paint, true, mRenderer.get().getMarginsColor());
		drawBackground(mRenderer.get(), canvas, x, y, width, margins[0], paint, true,
				mRenderer.get().getMarginsColor());
		if (or == Orientation.HORIZONTAL) {
			drawBackground(mRenderer.get(), canvas, x, y, left - x, height - y,
					paint, true, mRenderer.get().getMarginsColor());
			drawBackground(mRenderer.get(), canvas, right, y, margins[3], height - y,
					paint, true, mRenderer.get().getMarginsColor());
		} else if (or == Orientation.VERTICAL) {
			drawBackground(mRenderer.get(), canvas, right, y, width - right, height
					- y, paint, true, mRenderer.get().getMarginsColor());
			drawBackground(mRenderer.get(), canvas, x, y, left - x, height - y,
					paint, true, mRenderer.get().getMarginsColor());
		}

		boolean showLabels = mRenderer.get().isShowLabels() && hasValues;
		boolean showGridX = mRenderer.get().isShowGridX();
		// boolean showCustomTextGridX = mRenderer.isShowCustomTextGridX();
		boolean showCustomTextGridY = mRenderer.get().isShowCustomTextGridY();
		if (showLabels || showGridX) {
			List<Double> xLabels = getValidLabels(getXLabels(minX[0], maxX[0],
					mRenderer.get().getXLabels()));
			Map<Integer, List<Double>> allYLabels = getYLabels(minY, maxY,
					maxScaleNumber);

			int xLabelsLeft = left;
			if (showLabels) {
				paint.setColor(mRenderer.get().getXLabelsColor());
				paint.setTextSize(mRenderer.get().getLabelsTextSize());
				paint.setTextAlign(mRenderer.get().getXLabelsAlign());
				// if (mRenderer.getXLabelsAlign() == Align.LEFT) {
				// xLabelsLeft += mRenderer.getLabelsTextSize() / 4;
				// }
			}
			drawXLabels(xLabels, mRenderer.get().getXTextLabelLocations(), canvas,
			...
			if (showLabels) {
				paint.setColor(mRenderer.get().getLabelsColor());
				for (int i = 0; i < maxScaleNumber; i++) {
					Align axisAlign = mRenderer.get().getYAxisAlign(i);
					Double[] yTextLabelLocations = mRenderer.get()
					...
					for (Double location : yTextLabelLocations) {
						if (minY[i] <= location && location <= maxY[i]) {
							...
							String label = mRenderer.get().getYTextLabel(location, i);
							paint.setColor(mRenderer.get().getYLabelsColor(i));
							paint.setTextAlign(mRenderer.get().getYLabelsAlign(i));
							if (or == Orientation.HORIZONTAL) {
								if (axisAlign == Align.LEFT) {
									...
									drawText(
											...
											paint, mRenderer.get().getYLabelsAngle());
								} else {
								...
								if (showCustomTextGridY) {
									paint.setColor(mRenderer.get().getGridColor(i));
			...
			if (showLabels) {
				paint.setColor(mRenderer.get().getLabelsColor());
				float size = mRenderer.get().getAxisTitleTextSize();
				paint.setTextSize(size);
				paint.setTextAlign(Align.CENTER);
				if (or == Orientation.HORIZONTAL) {
					drawText(canvas, mRenderer.get().getXTitle(), x + width / 2,
							bottom + mRenderer.get().getLabelsTextSize() * 4 / 3
									+ mRenderer.get().getXLabelsPadding() + size,
							paint, 0);
					for (int i = 0; i < maxScaleNumber; i++) {
						Align axisAlign = mRenderer.get().getYAxisAlign(i);
						if (axisAlign == Align.LEFT) {
							drawText(canvas, mRenderer.get().getYTitle(i), x + size,
									y + height / 2, paint, -90);
						} else {
							drawText(canvas, mRenderer.get().getYTitle(i), x + width,
									y + height / 2, paint, -90);
						}
					}
					paint.setTextSize(mRenderer.get().getChartTitleTextSize());
					drawText(canvas, mRenderer.get().getChartTitle(), x + width / 2,
							y + mRenderer.get().getChartTitleTextSize(), paint, 0);
				} else if (or == Orientation.VERTICAL) {
					drawText(canvas, mRenderer.get().getXTitle(), x + width / 2, y
							+ height - size + mRenderer.get().getXLabelsPadding(),
							paint, -90);
					drawText(canvas, mRenderer.get().getYTitle(), right + 20, y
							+ height / 2, paint, 0);
					paint.setTextSize(mRenderer.get().getChartTitleTextSize());
					drawText(canvas, mRenderer.get().getChartTitle(), x + size, top
							+ height / 2, paint, 0);
				}
			}
		}
		if (or == Orientation.HORIZONTAL) {
			drawLegend(canvas, mRenderer.get(), titles, left, right, y
					+ (int) mRenderer.get().getXLabelsPadding(), width, height,
		...
		paint.setColor(mRenderer.get().getAxesColor());
		canvas.drawLine(left, bottom, right, bottom, paint);
		if (mRenderer.get().isShowAxes()) {
			// paint.setColor(mRenderer.getAxesColor());
			// canvas.drawLine(left, bottom, right, bottom, paint);
			boolean rightAxis = false;
			for (int i = 0; i < maxScaleNumber && !rightAxis; i++) {
				rightAxis = mRenderer.get().getYAxisAlign(i) == Align.RIGHT;
			}
			if (or == Orientation.HORIZONTAL) {
		...
		for (int i = 0; i < sLength; i++) {
			...
			hasValues = true;
			XYSeriesRenderer seriesRenderer = (XYSeriesRenderer) mRenderer.get()
			...
			synchronized (series) {
				SortedMap<Double, Double> range = series.getRange(minX[scale],
						maxX[scale], seriesRenderer.isDisplayBoundingPoints());
				int startIndex = -1;
				...
				for (Entry<Double, Double> value : range.entrySet()) {
					...
					if (startIndex < 0
							&& (!isNullValue(yValue) || isRenderNullValues())) {
					...
					if (!isNullValue(yValue)) {
						...
						if (points.size() > 0) {
							drawSeries(series, canvas, paint, points,
									seriesRenderer, yAxisValue, i, or,
									startIndex);
							ClickableArea[] clickableAreasForSubSeries = clickableAreasForPoints(
									points, values, yAxisValue, i, startIndex);
				...
				if (count > 0) {
					...
					for (int j = 0; j < count; j++) {
						...
						paint.getTextBounds(series.getAnnotationAt(j), 0,
								series.getAnnotationAt(j).length(), bound);
						if (xS < (xS + bound.width())
								&& yS < canvas.getHeight()) {
							drawString(canvas, series.getAnnotationAt(j), xS,
									yS, paint);
						}
					}
				}
		...
		// 画基准线
		paint.setColor(mRenderer.get().getAlignLineColor());
		paint.setTextSize(mRenderer.get().getLabelsTextSize());
		if (mRenderer.get().getAlign().length > 0) {
			for (int i = 0; i < mRenderer.get().getAlign().length; i++) {
				float label = mRenderer.get().getAlign()[i];
				float yLabel = (float) (bottom - label * yPixelsPerUnit[0]);
				paint.setFakeBoldText(false);
				drawText(canvas, label + "",
						right - mRenderer.get().getYLabelsPadding(), yLabel
								- mRenderer.get().getYLabelsVerticalPadding(), paint,
						mRenderer.get().getYLabelsAngle());
				canvas.drawLine(left / 2, yLabel, right, yLabel, paint);
			}
			if (mRenderer.get().isRect()) {
				float label = mRenderer.get().getAlign()[0];
				float yLabel = (float) (bottom - label * yPixelsPerUnit[0]);

				float label1 = mRenderer.get().getAlign()[1];
				float yLabel1 = (float) (bottom - label1 * yPixelsPerUnit[0]);
				paint.setColor(mRenderer.get().getRectColor());
				paint.setStyle(Style.FILL_AND_STROKE);
				canvas.drawRect(left, yLabel1, right, yLabel, paint);
			}
		}
	}
	...
	protected Map<Integer, List<Double>> getYLabels(double[] minY,
		...
		for (int i = 0; i < maxScaleNumber; i++) {
			allYLabels.put(i, getValidLabels(MathHelper.getLabels(minY[i],
					maxY[i], mRenderer.get().getYLabels())));
		}
		return allYLabels;
	}
	...
	protected void drawSeries(XYSeries series, Canvas canvas, Paint paint,
		...
		if (isRenderPoints(seriesRenderer)) {
			...
			if (pointsChart != null) {
				pointsChart.drawSeries(canvas, paint, pointsList,
						seriesRenderer, yAxisValue, seriesIndex, startIndex);
			}
		}
		...
		if (seriesRenderer.isDisplayChartValues()) {
			...
			drawChartValuesText(canvas, series, seriesRenderer, paint,
					pointsList, seriesIndex, startIndex);
		}
		if (stroke != null) {
	...
	protected void drawChartValuesText(Canvas canvas, XYSeries series,
			XYSeriesRenderer renderer, Paint paint, List<Float> points,
		...
		if (points.size() > 1) { // there are more than one point
			...
			for (int k = 0; k < points.size(); k += 2) {
				if (k == 2) { // decide whether to display first two points'
								// values or
								// not
					if (Math.abs(points.get(2) - points.get(0)) > renderer
							.getDisplayChartValuesDistance()
							|| Math.abs(points.get(3) - points.get(1)) > renderer
									.getDisplayChartValuesDistance()) {
						int color = getColor(series.getY(startIndex));
						if (color != -1) {
							paint.setColor(color);
						}
						// first point
						if (series.getY(startIndex) != 0) {
							drawText(
									canvas,
									getLabel(renderer.getChartValuesFormat(),
											series.getY(startIndex)),
									points.get(0),
									points.get(1)
						...
						// second point
						color = getColor(series.getY(startIndex + 1));
						if (color != -1) {
							paint.setColor(color);
						}
						if (series.getY(startIndex + 1) != 0) {
							drawText(
									canvas,
									getLabel(renderer.getChartValuesFormat(),
											series.getY(startIndex + 1)),
									points.get(2),
									points.get(3)
											- renderer.getChartValuesSpacing(),
									paint, 0);
					...
					if (Math.abs(points.get(k) - previousPointX) > renderer
							.getDisplayChartValuesDistance()
							|| Math.abs(points.get(k + 1) - previousPointY) > renderer
									.getDisplayChartValuesDistance()) {
						int color = getColor(series.getY(startIndex + k / 2));
						if (color != -1) {
							paint.setColor(color);
						}
						if (series.getY(startIndex + k / 2) != 0) {
							drawText(
									canvas,
									getLabel(renderer.getChartValuesFormat(),
											series.getY(startIndex + k / 2)),
									points.get(k),
									points.get(k + 1)
											- renderer.getChartValuesSpacing(),
									paint, 0);
			...
			for (int k = 0; k < points.size(); k += 2) {
				if (points.get(k) != 0) {
					drawText(
							...
							points.get(k),
							points.get(k + 1)
									- renderer.getChartValuesSpacing(), paint,
							0);
				}
			}
		}
	...
	private int getColor(double value) {
		float align[] = mRenderer.get().getAlign();
		int[] alignColors = mRenderer.get().getAlignColors();
		if (align.length == 0) {
			return -1;
		}
		if (value < align[0]) {
			return alignColors[0];
		}
		if (value > align[align.length - 1]) {
			return alignColors[align.length];
		}
		for (int i = 0; i < align.length - 1; i++) {
			if (value > align[i] && value < align[i + 1]) {
				return alignColors[i + 1];
			}
		}
		return -1;
	}
	...
	protected void drawText(Canvas canvas, String text, float x, float y,
		...
		float angle = -mRenderer.get().getOrientation().getAngle() + extraAngle;
		if (angle != 0) {
	...
	protected void drawXLabels(List<Double> xLabels,
		...
		paint.setTextSize(mRenderer.get().getLabelsTextSize());
		boolean showLabels = mRenderer.get().isShowLabels();
		boolean showGridY = mRenderer.get().isShowGridY();
		for (int i = 0; i < length; i++) {
			...
			// xLabel, bottom + mRenderer.getLabelsTextSize() * 4 / 3
			// + mRenderer.getXLabelsPadding(), paint,
			// mRenderer.getXLabelsAngle());
			// }
			if (showGridY && i != 0) {
				paint.setColor(mRenderer.get().getGridColor(0));
	...
	protected void drawYLabels(Map<Integer, List<Double>> allYLabels,
		...
		for (int i = 0; i < maxScaleNumber; i++) {
			...
			for (int j = 0; j < length; j++) {
				...
				Align axisAlign = mRenderer.get().getYAxisAlign(i);
				boolean textLabel = mRenderer.get().getYTextLabel(label, i) != null;
				float yLabel = (float) (bottom - yPixelsPerUnit[i]
						* (label - minY[i]));
				if (or == Orientation.HORIZONTAL) {
					if (showLabels && !textLabel) {
						...
						if (axisAlign == Align.LEFT) {
							...
							drawText(
									canvas,
									getLabel(mRenderer.get().getLabelFormat(), label),
									left - mRenderer.get().getYLabelsPadding(),
									yLabel
											- mRenderer.get()
													.getYLabelsVerticalPadding(),
									paint, mRenderer.get().getYLabelsAngle());
							...
							drawText(
									canvas,
									getLabel(mRenderer.get().getLabelFormat(), label),
									right + mRenderer.get().getYLabelsPadding(),
									yLabel
											- mRenderer.get()
													.getYLabelsVerticalPadding(),
									paint, mRenderer.get().getYLabelsAngle());
					...
					if (showLabels && !textLabel) {
						...
						drawText(canvas,
								getLabel(mRenderer.get().getLabelFormat(), label),
								right + 10 + mRenderer.get().getYLabelsPadding(),
								yLabel - mRenderer.get().getYLabelsVerticalPadding(),
								paint, mRenderer.get().getYLabelsAngle());
					}
					if (showGridX) {
						paint.setColor(mRenderer.get().getGridColor(i));
	...
	protected void drawXTextLabels(Double[] xTextLabelLocations, Canvas canvas,
		...
		boolean showCustomTextGridX = mRenderer.get().isShowCustomTextGridX();
		if (showLabels) {
			paint.setColor(mRenderer.get().getXLabelsColor());
			for (Double location : xTextLabelLocations) {
				if (minX <= location && location <= maxX) {
					...
					paint.setColor(mRenderer.get().getXLabelsColor());
					canvas.drawLine(xLabel, bottom, xLabel,
							bottom + mRenderer.get().getLabelsTextSize() / 3, paint);
					paint.setTextAlign(mRenderer.get().getXLabelsAlign());
					drawText(canvas, mRenderer.get().getXTextLabel(location), xLabel,
							bottom + mRenderer.get().getLabelsTextSize() * 4 / 3,
							paint, mRenderer.get().getXLabelsAngle());
	...
	public double[] toRealPoint(float screenX, float screenY, int scale) {
		double realMinX = mRenderer.get().getXAxisMin(scale);
		double realMaxX = mRenderer.get().getXAxisMax(scale);
		double realMinY = mRenderer.get().getYAxisMin(scale);
		double realMaxY = mRenderer.get().getYAxisMax(scale);
		if (!mRenderer.get().isMinXSet(scale) || !mRenderer.get().isMaxXSet(scale)
				|| !mRenderer.get().isMinXSet(scale) || !mRenderer.get().isMaxYSet(scale)) {
		...
		if (mScreenR != null) {
			return new double[] {
					...
					(mScreenR.top + mScreenR.height() - screenY)
							* (realMaxY - realMinY) / mScreenR.height()
							+ realMinY };
		} else {
						...
						if (area != null) {
							...
							if (rectangle != null
									&& rectangle.contains(screenPoint.getX(),
											screenPoint.getY())) {
								return new SeriesSelection(seriesIndex,
										pointIndex, area.getX(), area.getY());
							}
						}
						pointIndex++;
					}
	...
	protected abstract ClickableArea[] clickableAreasForPoints(
			List<Float> points, List<Double> values, float yAxisValue,
			int seriesIndex, int startIndex);
