package com.zfg.org.myexample.widget.wheel;
...
public class WheelView extends View {
	...
	private static final int VALUE_TEXT_COLOR = 0xFF4BC4D9;

	/** Items text color */
	private static final int ITEMS_TEXT_COLOR = 0xFF4BC4D9;
	...
	private void initResourcesIfNecessary() {	
		if (itemsPaint == null) {
			...
			itemsPaint.setTextSize(scale * TEXT_SIZE);
		}

		if (valuePaint == null) {
			valuePaint = new TextPaint(Paint.ANTI_ALIAS_FLAG | Paint.DITHER_FLAG);
			//valuePaint.density = getResources().getDisplayMetrics().density;
			valuePaint.setTextSize(scale * TEXT_SIZE);
			valuePaint.setShadowLayer(0.1f, 0, 0.1f, 0xFFC0C0C0);
		}

		if (centerDrawable == null) {
			centerDrawable = getContext().getResources().getDrawable(R.drawable.wheel_val_holo);
		}
		...
		//setBackgroundResource(R.drawable.wheel_bg_holo);
	}
	...
	private int getDesiredHeight(Layout layout) {
		...
		int desired = getItemHeight() * visibleItems - (int)(ITEM_OFFSET *scale * 2)
...
//			labelWidth = (int) FloatMath.ceil(Layout.getDesiredWidth(label, valuePaint));
	...
	protected void onDraw(Canvas canvas) {
		...
		if (itemsWidth > 0) {
			...
			canvas.translate(PADDING, scale*-ITEM_OFFSET);
			drawItems(canvas);
			drawValue(canvas);
			canvas.restore();
		}
